...
export default function Post({post}: PostProps) {
    ...
    const handleLike = async () => {
        try {
            const postIsLiked = await toggleLike({ postId: post._id });
            setIsLiked(postIsLiked);
            setLikesCount((prev) => (postIsLiked ? prev + 1 : prev - 1));
        } catch (error) {
        ...
        <View style={styles.post}>
            ...
            <View style={styles.postHeader}>
                <Link href={"/(tabs)/notifications"}>
                    ...
                    {/* IF OWNER OF POST, SHOW DELETE OPTIONS */}
            ...
            <View style={styles.postInfo}>
                <Text style={styles.likesText}>
                    {likesCount > 0 ? `${likesCount.toLocaleString()} likes` : "Be the first to like this post"}
                </Text>
                {post.caption && (
